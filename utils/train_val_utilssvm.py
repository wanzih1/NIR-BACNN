...
from Lenet5 import MyDataset
from densenet import densenet121
import tensorflow as tf
from Lenet5 import VGG16
import random
from Lenet5 import D2
from Lenet5 import BPNet
from Lenet5 import ALEXNet
from Lenet5 import Lenet
...
def loss(output, y_input, readout_weight, batch_size, num_classes, penalty_parameter):
    ...
    loss = egularization_loss + penalty_parameter * hinge_loss
    return loss
...
def evaluate(model, dataloader, device, epoch):
    ...
    for i, data in enumerate(dataloader):
        ...
        dataloader.desc = ("valid:epoch = {:} Loss = {:.4f}  Acc= {:.4f}".format((epoch + 1), (loss1.item()),
                                                                    (acc)))  # 训练次数，总损失，精确度
    return loss1.item(), correct1 / total1, out_features, labels1